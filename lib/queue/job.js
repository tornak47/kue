...
var search =null;// reds.createSearch('q:search');
...
exports.rangeByType = function(type, status, from, to, order, fn){
  pool.alloc().from('jobs').where('type',type).where('status',status).limit(from,to).select('id').execute(get(fn, order));
};
...
exports.get = function(id, fn){
  ...
  job.id = id;
  client.from('jobs').where('id',job.id).execute(function(err, hash){
    if (err) return fn(err);
